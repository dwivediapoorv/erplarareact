...
interface ByProjectProps {
    project: { id: number; name: string; health: 'Red' | 'Green' | 'Orange' };
    tasks: Task[];
}
...
export default function TasksByProject({ project, tasks: taskList }: ByProjectProps) {
    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Dashboard', href: '/dashboard' },
        { title: `${project.name} — Open Tasks`, href: '#' },
    ];

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`${project.name} — Open Tasks`} />
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <div className="flex items-center gap-2">
                            <span className={`w-2.5 h-2.5 rounded-full ${healthDot[project.health]}`} />
                            <h1 className="text-2xl font-semibold">{project.name}</h1>
                            <span className={`text-sm font-medium ${healthText[project.health]}`}>
                                {project.health}
                            </span>
                        </div>
                        ...
                        {taskList.map((task) => (
                            ...
                            >
                                ...
                                <div className="flex items-center justify-between mt-3 pt-3 border-t border-sidebar-border/30 text-xs text-muted-foreground">
                                    <span><span className="font-medium">Assigned to:</span> {task.assignee_name}</span>
